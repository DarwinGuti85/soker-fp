...
const ServiceDetailsView: React.FC<{ service: ServiceOrder, onClose: () => void }> = ({ service, onClose }) => {
    const { inventory, updateService, updateInventoryItem, invoices, users } = useData();
    ...
    const [newNotes, setNewNotes] = useState(service.technicianNotes);
    const [isAddingPart, setIsAddingPart] = useState(false);
    const [partSearch, setPartSearch] = useState('');
    const [isEditingTechnician, setIsEditingTechnician] = useState(false);
    const [selectedTechnicianId, setSelectedTechnicianId] = useState(service.technician?.id || '');
    ...
    const handleSaveNotes = () => {
        if(newNotes !== undefined) {
            updateService(service.id, { technicianNotes: newNotes });
    ...
    const handleSaveTechnician = () => {
        ...
        updateService(service.id, { technician: technicianToAssign });
        setIsEditingTechnician(false);
    };

    const handleAddPart = (part: InventoryItem) => {
        if (part.quantity > 0) {
            const updatedParts = [...(service.partsUsed || []), part];
            updateService(service.id, { partsUsed: updatedParts });
    ...
    const handleRemovePart = (partToRemove: InventoryItem) => {
        const updatedParts = service.partsUsed?.filter(p => p.id !== partToRemove.id);
    ...
    const availableParts = useMemo(() => {
        const usedPartIds = new Set(service.partsUsed?.map(p => p.id));
    ...
    }, [inventory, service.partsUsed, partSearch]);

    const handleBillService = () => {
        navigate(`/billing?action=create&serviceId=${service.id}`);
        onClose();
    };

    const isCompleted = service.status === ServiceStatus.COMPLETED;
    const existingInvoice = invoices.find(inv => inv.serviceOrder?.id === service.id);
    ...
    return (
        <div className="bg-brand-orange/10 dark:bg-brand-bg-light rounded-lg shadow-lg border border-brand-orange/30 dark:border-gray-700/50 animate-fadeInUp">
            ...
            <div className="px-6 py-4 border-b border-brand-orange/20 dark:border-gray-700 flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Detalles del Servicio #{service.originalInvoiceNumber || service.id}</h2>
                <button onClick={onClose} className="bg-gray-600 text-white px-4 py-2 rounded-md font-semibold hover:bg-gray-500 transition-colors"> Volver a la lista </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-grow space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {!isEditingStatus ? (
                        <div className="flex items-center gap-2">
                            <span className={`px-3 py-1 text-sm leading-5 font-semibold rounded-full ${STATUS_COLORS[service.status]}`}>{STATUS_LABELS_ES[service.status]}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                        <h3 className="text-lg font-bold text-brand-orange border-b border-brand-orange/30 pb-2 flex justify-between items-center">
                            <span>Cliente</span>
                            {service.client.whatsapp && <a href={`https://wa.me/${service.client.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 flex items-center gap-2 bg-green-600 text-white px-3 py-1 rounded-md font-semibold hover:bg-green-500 transition-colors text-xs"><WhatsAppIcon className="h-4 w-4" /><span>Contactar</span></a>}
                        </h3>
                        <p><strong className="text-brand-text-dark">Nombre:</strong> {`${service.client.firstName} ${service.client.lastName}`}</p>
                        <p><strong className="text-brand-text-dark">Email:</strong> {service.client.email || 'No tiene'}</p>
                    </div>
                    {/* Appliance Info */}
                    <div className="space-y-2">
                         <h3 className="text-lg font-bold text-brand-orange border-b border-brand-orange/30 pb-2">Artefacto</h3>
                        <p><strong className="text-brand-text-dark">Tipo:</strong> {service.applianceName}</p>
                        <p><strong className="text-brand-text-dark">Marca/Modelo:</strong> {service.applianceType}</p>
                        {service.originalInvoiceNumber && <p><strong className="text-brand-text-dark">ID Interno:</strong> {service.id}</p>}
                    </div>
                </div>
                       ...
                       {!isEditingTechnician ? (
                            <div className="flex items-center gap-2">
                                <p className="text-brand-text">
                                    {service.technician ? `${service.technician.firstName} ${service.technician.lastName}` : 'Sin asignar'}
                                </p>
                ...
                <div className="space-y-2">
                    ...
                    <div>
                        ...
                        <p className="text-brand-text-dark bg-brand-bg-dark/50 p-3 rounded-md whitespace-pre-wrap mt-1">{service.clientDescription}</p>
                    </div>
                    <div>
                        ...
                        {isEditingNotes ? (
                            ...
                            <p className="text-brand-text-dark bg-brand-bg-dark/50 p-3 rounded-md whitespace-pre-wrap min-h-[6rem]">{service.technicianNotes || 'Sin notas.'}</p>
                        )}
                    </div>
                </div>
                     ...
                     <div className="overflow-x-auto rounded-lg border border-gray-700 mt-2">
                         <table className="min-w-full text-sm">
                             ...
                             <tbody className="divide-y divide-gray-700">
                                 {service.partsUsed && service.partsUsed.length > 0 ? service.partsUsed.map(part => (
                                     <tr key={part.id}>
            ...
            {/* Footer */}
            <div className="px-6 py-4 border-t border-brand-orange/20 dark:border-gray-700 flex justify-end items-center space-x-3">
...
const NewOrderView: React.FC<{
  ...
  return (
    <div className="bg-brand-orange/10 dark:bg-brand-bg-light rounded-lg shadow-lg border border-brand-orange/30 dark:border-gray-700/50 animate-fadeInUp">
        <div className="px-6 py-4 border-b border-brand-orange/20 dark:border-gray-700 flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Nueva Orden de Servicio</h2>
            <button onClick={onClose} className="bg-gray-600 text-white px-4 py-2 rounded-md font-semibold hover:bg-gray-500 transition-colors"> Volver a la lista </button>
        </div>
...
const Services: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeInUp">
      ...
      {isFormVisible ? (
          <NewOrderView onClose={() => setIsFormVisible(false)} />
      ) : selectedService ? (
        <ServiceDetailsView service={selectedService} onClose={handleCloseDetails} />
      ) : (
        <>
