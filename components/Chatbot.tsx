...
import { ChatBubbleOvalLeftEllipsisIcon, CloseIcon, PaperAirplaneIcon, SparklesIcon } from './ui/icons';
...
6.  **FORMATO:** Utiliza formato Markdown (listas, **negritas**) para que tus respuestas sean claras.`;
        ...
        setMessages([{
            role: 'model',
            text: '¡Hola! Soy chupamestepenco AI. ¿En qué puedo ayudarte hoy?'
        }]);
    };
    ...
    const prepareDataContext = (): string => {
        ...
        const summary = {
            ...
            // Include contact details for clients
            ...
            // Include client name in services
            ...
            // Only show low stock items to keep the context concise
    ...
    const handleSendMessage = async (e: FormEvent) => {
        ...
        try {
            ...
            // Using marked to parse markdown response
    ...
    return (
        <>
            <button className="chat-bubble" onClick={() => setIsOpen(true)} aria-label="Abrir asistente virtual">
                <SparklesIcon className="w-8 h-8 text-white" />
            </button>
            <div className={`chat-panel ${isOpen ? animationClass : 'chat-panel-hidden'}`}>
                <header className="flex items-center justify-between p-4 bg-brand-bg-dark border-b border-gray-700">
                    <div className="flex items-center gap-2">
                        <SparklesIcon className="w-6 h-6 text-brand-orange" />
                        <h2 className="font-bold text-white text-lg">chupamestepenco AI</h2>
                    </div>
                    <button onClick={() => setIsOpen(false)} className="text-brand-text-dark hover:text-white">
                ...
                <div className="p-4 border-t border-gray-700 bg-brand-bg-dark">
                    <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                        <input
                            ...
                            className="flex-grow bg-brand-bg-light border border-gray-600 rounded-full py-2 px-4 text-brand-text focus:ring-2 focus:ring-brand-orange focus:outline-none"
                            disabled={isLoading}
                            autoComplete="off"
                        />
